...
import { BoardMember, ActiveTab, Meeting, InAppNotification, Resolution, VoteType } from '../types';
import { 
  Vote, 
  Receipt, 
  LayoutDashboard, 
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40 pt-[env(safe-area-inset-top)]">
      {/* Top Banner: Upcoming Online Meeting */}
      {upcomingMeeting && (
        <div className="bg-[#00266B] text-white px-4 py-2 text-xs border-b border-blue-900">
          <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2.5">
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <span className="font-semibold text-white">
                {new Date(upcomingMeeting.date).toLocaleDateString('de-DE', {
                  weekday: 'short',
                  day: '2-digit',
                  month: '2-digit',
                  year: 'numeric',
                })} • {upcomingMeeting.startTime} Uhr
              </span>
              <span className="hidden md:inline text-blue-200 text-[11px] truncate max-w-xs">
                ({upcomingMeeting.title})
              </span>
            </div>

            {/* Right: Direct Actions (Agenda & Teams) */}
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={onOpenQuickAgenda}
                className="px-2.5 py-1 bg-white/10 hover:bg-white/20 text-white rounded-lg font-bold text-[11px] flex items-center space-x-1.5 transition-colors cursor-pointer border border-white/15"
                title="Tagesordnungspunkte sofort ansehen"
              >
                <FileText className="w-3.5 h-3.5 text-[#00A3E0]" />
                <span>Agenda aufrufen ({upcomingMeeting.agenda.length} TOPs)</span>
              </button>

              {upcomingMeeting.teamsUrl && (
                <a
                  href={upcomingMeeting.teamsUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-2.5 py-1 bg-[#464EB8] hover:bg-[#3B429F] text-white rounded-lg font-bold text-[11px] flex items-center space-x-1 transition-colors shadow-xs"
                >
